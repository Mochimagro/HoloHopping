...
namespace ArborEditor
{
	...
	public static class ClassList
	{
		...
		{
			if (a == null || TypeUtility.IsDynamic(a))
			{
				return false;
			}
		...
		internal static ReadOnlyCollection<Assembly> GetAssemblies()
		{
			...
			for (int i = 0; i < pluginImporters.Length; i++)
			{
				PluginImporter pluginImporter = pluginImporters[i];
			...
			DebugApplyLoaded(); // Conditional("ARBOR_CLASSLIST_DEBUG")
#else
			Debug("[CurrentDomain.GetAssemblies]"); // Conditional("ARBOR_CLASSLIST_DEBUG")
