...
#pragma warning restore 219
		...
		private static bool IsDefined(Assembly assembly, Type attributeType)
		{
#if NETFX_CORE
			return assembly.IsDefined(attributeType);
#else
		...
		{
			return IsDefined(assembly, typeof(UnityEngine.AssemblyIsEditorAssembly)) ||
		...
		private static bool IsEditorAssembly(AssemblyName assemblyName)
		{
			...
			return name == "Assembly-CSharp-Editor" || name == "Assembly-CSharp-Editor-firstpass" || name == "UnityEditor";
		}
		...
		public static async System.Threading.Tasks.Task<List<Assembly>> GetAssemblyList()
		{
			...
			for (int i = 0; i < files.Count; i++)
			{
				...
				var assembly = TryToLoad(new AssemblyName(file.DisplayName));
				if (assembly != null)
				{
		...
		static Assembly[] GetAssembies()
		{
#if WINDOWS_UWP
			return GetAssemblyList().Result.ToArray();
#else
			...
			if (assemblies != null)
			{
				for (int i = 0; i < assemblies.Count; i++)
				{
					...
					for (int typeIndex = 0; typeIndex < types.Length; typeIndex++)
					{
						...
						if (AttributeHelper.HasAttribute<System.Runtime.CompilerServices.CompilerGeneratedAttribute>(GetMemberInfo(type)))
						{
							continue;
						}
		...
		public static Type GetTypeRenamedFrom(string typeName)
		{
			...
			if (s_RenamedTypes == null)
			{
				...
				for (int typeIndex = 0; typeIndex < types.Count; typeIndex++)
				{
					...
					var attributes = AttributeHelper.GetAttributes<RenamedFromAttribute>(GetMemberInfo(type));
